...
Marco Aurélio Kawassaki Nakasima 07N
41410483

TCC
Clusterização: Uma breve introdução sobre conceitos e algoritmos
...
Um breve exemplo sobre clusterização pode ser representada em um simples gráfico:

 
Nesse caso, identificamos facilmente os 4 clusters nos quais os dados podem ser divididos; O critério de similaridade é a distância: dois ou mais objetos pertencem ao mesmo cluster se eles são "próximos" de acordo com uma determinada distância (neste caso distância geométrica). Isso é chamado de cluster baseado em distância.


Objetivos da clusterização de dados
	Como já dito acima, o principal objetivo é determinar e agrupar um conjunto de dados crus. Mas como decidir o fator de utilidade e quão bom um algoritmo de clusterização é bom? Podemos com certeza afirmar que não existe um critério ideal para clusterizar dados. Consequentemente, podemos afirmar que é o usuário que deve suprir esse critério, de maneira a escolher um algoritmo que mais se aproxima do resultado desejado.
